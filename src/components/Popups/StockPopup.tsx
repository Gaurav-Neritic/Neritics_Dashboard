...
const StockPopup = ({ isVisible, onClose, productId, productName, totalStock }: stockPopupProps) => {
    const [newStock, setNewStock] = useState(totalStock);
    const queryClient = useQueryClient();

    async function editStock() {
        try {
            const response = await axios.put("../api/editStock", { productId, newStock });
            ...
            <div className="bg-white dark:bg-neutral-800 border border-neutral-700 p-6 rounded-lg shadow-lg max-w-md w-full">
                ...
                >
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        >
                            {editStockMutation.isPending ? <Loader title='Updating...' /> : "Update Stock"}
                        </button>
                    </div>
                </form>
            </div>
